...
class TransformTests(unittest.TestCase):
    def test_html_text_keeps_semantic_lines(self) -> None:
        self.assertEqual(clean_text("<p>Первый&nbsp;текст</p><ul><li>Второй</li></ul>"), "Первый текст\nВторой")
    ...
    def test_program_is_nested_under_department_and_choice_is_explicit(self) -> None:
            ...
            "faculties": [
                {
                    ...
                    "chairs": [{"slug": "test-chair", "code": "К", "title": "Тестовая кафедра"}],
                }
            ],
        }
        detail = {
        ...
        major = Normalizer().normalize(DetailFetch(summary, detail, None, "2026-01-01T00:00:00+00:00"))
        self.assertEqual(major.status, "ok")
        self.assertEqual(len(major.departments), 1)
        self.assertEqual(major.departments[0].educational_programs[0].department_id, major.departments[0].id)
        self.assertEqual(major.entrance_requirements[0].requirement_type, "обязательный")
