...
class ApiClient:
    """Small resilient HTTP client shared by the API and plan resolvers."""

    def __init__(self, timeout: float = 30.0, delay: float = 0.15) -> None:
    ...
    def _session(self) -> requests.Session:
        ...
        if session is None:
            ...
            session.headers.update(
                {
                    "User-Agent": "bmstu-education-parser/2.0 (+https://mirror.bmstu.ru)",
    ...
    def get_json(self, url: str, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            raise FetchError(f"GET {url}: ответ не является JSON") from exc
        if not isinstance(payload, dict):
            raise FetchError(f"GET {url}: ожидался JSON-объект")
        return payload
