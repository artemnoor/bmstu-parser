...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--result", type=Path, default=env.result_dir, help="Каталог результатов parser"
    )
    ...
    parser.add_argument(
        "--api-key",
        default=None,
        help="API key для write endpoints; по умолчанию BMSTU_API_KEY",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    settings = replace(
        ...
        host=args.host,
        port=args.port,
        api_key=ApiSettings.from_env().api_key
