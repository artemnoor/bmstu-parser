...
class StudyPlanOntologyBuilder:
    ...
    def add_object(self, object_type: str, identifier: str, properties: dict[str, Any], provenance: dict[str, Any]) -> str:
    ...
    def add_link(self, link_type: str, from_id: str, to_id: str, provenance: dict[str, Any]) -> str:
    ...
    def build(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        for result in results:
            ...
            for reference in references:
                program_id = reference.get("program_id") or stable_id("educational-program", reference.get("program_name"))
                self.add_object(
                    "educational_program",
                    program_id,
                    {
                ...
                self.add_link("study_plan_document_for_program", document_object, program_id, provenance)
            for table in result.get("tables", []):
                ...
                self.add_link("study_plan_document_contains_table", document_object, table_object, provenance)
                for row_index, cells in enumerate(table.get("rows", [])):
                    ...
                    row_object = self.add_object(
                        ...
                        row_identifier,
                        {
                            ...
                            "cells_locator": {"table_id": table["id"], "row_index": row_index},
                        },
                        provenance,
                    )
                    self.add_link("study_plan_table_contains_row", table_object, row_object, provenance)
        return {
            "objects": {
